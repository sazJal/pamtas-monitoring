...
//	disp_spi_transfer_addrwin(x, x+10, y, y+10);
...
// ** Device must already be selected and address window set **
//--------------------------------------------------------------------------------
static void IRAM_ATTR _TFT_pushColorRepImage(uint32_t *imageBuf, uint32_t len)
{
	if (!(disp_spi->cfg.flags & SPI_DEVICE_HALFDUPLEX)) return;
...
// uses the buffer to fill the color values
//-------------------------------------------------------------------------------------------
void IRAM_ATTR TFT_pushColorImageRep(int x1, int y1, int x2, int y2, uint32_t *imageBuf, uint32_t len)
{
	if (disp_select() != ESP_OK) return;
	...
	_TFT_pushColorRepImage(imageBuf, len);
	disp_deselect();
}
